  ...
  <div>
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center">
        ...
        <button type="button" @click="deleteQuestion()"
          class="flex items-center text-xs py-1 px-3 border border-transparent text-red-500 hover:border-red-600 hover:bg-gray-700">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            stroke="currentColor" class="w-6 h-6">
    ...
    <div class="grid gap-3 grid-cols-12">
      ...
      <div class="mt-3 col-span-3">
        ...
        <select name="question_type" id="question_type" v-model="model.type" @change="typeChange()"
          ...
          <option v-for="q_type in questionTypes" :key="q_type" :value="q_type">
            {{ upperCaseFirst(q_type) }}
          </option>
        </select>
      </div>
      <!-- End Question Type -->
    ...
    <div v-for="(option, index) in model.data.options" :key="option.uuid" class="flex items-center mb-1">
      ...
      <button type="button" @click="removeOption(option)"
        class="h-6 w-6 rounded-full flex items-center border border-transparent transition-colors hover:border-red-100">
